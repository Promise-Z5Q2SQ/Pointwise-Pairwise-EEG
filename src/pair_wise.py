...
def generate_pair(total_arr):
    pairs = []
    for tmp_dic in total_arr:
        for idx in range(len(tmp_dic['image']) // 2):
            pairs.append({
                "query": tmp_dic['query'],
                "intent": tmp_dic['intent'],
                "image": tmp_dic['image'][idx * 2:idx * 2 + 2]
            })
    return pairs


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='pair-wise preference research with eeg')
    ...
    total_pair = generate_pair(total_arr)
    main_function(total_pair,
                  open('./record/' + datetime.now().strftime('%Y%m%d') + '_' + str(args.seed), 'w', encoding='utf8'))
